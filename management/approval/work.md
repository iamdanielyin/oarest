#汇报列表
>先按审批状态排序，未审的在前面；再按创建时间逆序排列；每个用户只能查询到和自己相关的审批信息。我向上级汇报的任务和我查看下级的汇报记录都是通过该接口来获取数据，通过不同页签加以区分。
...
|编码|名称|类型|必输|说明|默认值|
|:---|:---|:---|:--:|:---|:-----|
|data|工作数组|<code>array</code>|是|暂无|无|
...
|modifytime|最近修改时间|<code>string</code>|是|暂无|无|attach
